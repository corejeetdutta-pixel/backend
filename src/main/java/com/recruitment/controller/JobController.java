 ...
 // Helper method to extract user from JWT token
    ...
    public ResponseEntity<?> getApplicantsForJob(@PathVariable String jobId, HttpServletRequest request) {
        ...
        try {
            ...
            if (jobOpt.isEmpty()) {
                return ResponseEntity.ok(new ArrayList<>()); // Return empty array instead of 404
            }
            ...
            List<Map<String, Object>> applicants = applications != null ? applications.stream().map(app -> {
    ...
    // New endpoint for admin to get applicants for any job
 // New endpoint for admin to get applicants for any job
    ...
    public ResponseEntity<?> getApplicantsForJobAdmin(@PathVariable String jobId, HttpServletRequest request) {
        System.out.println("Admin applicants fetch triggered");
